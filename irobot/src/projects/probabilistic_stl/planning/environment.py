...
# =============================================================================
# EXPERIMENT ENVIRONMENT FACTORIES
# =============================================================================


def build_test4_environment(device="cpu") -> Environment:
    """
    Test 4: Point-to-point flight. No obstacles.
    All coordinates in meters from CF home position (lighthouse origin).

    Layout:
      Start:  (0.0, 0.0)  — CF powers on here
      Goal:   x=[1.3, 1.7], y=[-0.2, 0.2]  — 1.5m forward, centered

    MEASURE BEFORE EACH RUN: confirm goal marker is placed at ~1.5m from
    CF power-on position along the x-axis of the lighthouse frame.
    """
    env = Environment(device=device)
    env.set_goal(x_range=[1.3, 1.7], y_range=[-0.2, 0.2])
    return env


def build_test5_environment(device="cpu") -> Environment:
    """
    Test 5: Obstacle avoidance flight.
    A single box obstacle placed between start and goal.

    Layout:
      Start:   (0.0, 0.0)
      Obstacle: x=[0.6, 1.0], y=[-0.3, 0.3]  — box at ~0.8m forward
      Goal:    x=[1.6, 2.0], y=[-0.2, 0.2]   — 1.8m forward

    MEASURE BEFORE EACH RUN: place physical box at marked floor position.
    """
    env = Environment(device=device)
